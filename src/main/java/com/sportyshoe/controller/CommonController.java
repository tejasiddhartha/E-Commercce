...
import com.sportyshoe.model.UserInfoModel;
import com.sportyshoe.service.OrdersService;
	...
	*/
	...
	public String requiredReportsMethod(ModelMap model, @RequestParam String categeory, @RequestParam String date) {
		...
		try {
			...
			if (categeoryId != 0) {
				orderedShoeList = orderservice.getRequiredCompleteTransactionsDataService(categeoryId, date);
				if (orderedShoeList != null) {
					double totalSales = 0;
					for (OrderedShoeModel osl : orderedShoeList) {
						totalSales = totalSales + osl.getTotalprice();
